 ...
 * Auxiliary Space: O(1)
 *
 * Used in practice to heapSort almost sorted array with complexity of 2n e.g. polygon filling algorithm
 */
class BubbleSort {
    ...
    static void bubbleSort(int arr[]) {
        int n = arr.length;
        // swap temp and arr[i]
        IntStream.range(0, n - 1).forEach(i -> IntStream.range(0, n - i - 1).filter(j -> arr[j] > arr[j + 1]).forEach(j -> {
            int temp = arr[j];
            arr[j] = arr[j + 1];
            arr[j + 1] = temp;
        }));
