...
// given a list of projects and a list of pair of dependencies, determine the build order of the projects
// or return error if not possible
public class BuildOrder {

    // first approach towards this would be that we represent the projects as a graph node
    // and draw the dependencies between them with edges going from independent project to dependent project
    ...
    private Graph buildGraph(String[] projects, String[][] dependencies) {
        ...
        Arrays.stream(dependencies).forEach(dependency -> graph.addEdge(dependency[0], dependency[1])); // create the dependencies and map
        return graph;
    }
}