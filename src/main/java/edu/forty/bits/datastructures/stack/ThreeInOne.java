...
/**
 * Describe how could you implement 3 stacks using a single array.
 */
public class ThreeInOne {
    ...
    private int numberOfElementsStream() {
        return Arrays.stream(info).mapToInt(sd -> sd.size).sum();
    }
    ...
    private void shift(int stackNum) {
        ...
        if (stack.size >= stack.capacity) {
            int nextStack = (stackNum + 1) % info.length; // round about from max to min again
            shift(nextStack);
