...
public class OneValueOps {
    ...
    private static int findLeastFrequentLargestElement(Map<Integer, Integer> hashMap) {
        ...
        Map<Integer, Integer> sortedByValueMap = list.stream().collect(
        ...
        List<Map.Entry<Integer, Integer>> x =
                sortedByValueMap.entrySet().stream().filter(m -> m.getValue() == leastFrequentValue).collect(Collectors.toList());
    ...
    private static int findMostFrequentSmallestElement(Map<Integer, Integer> hashMap) {
        ...
        List<Map.Entry<Integer, Integer>> x =
                sortedByValueMap.entrySet().stream().filter(m -> m.getValue() == mostFrequentValue).collect(Collectors.toList());
        for(Map.Entry<Integer, Integer> a : x) {
