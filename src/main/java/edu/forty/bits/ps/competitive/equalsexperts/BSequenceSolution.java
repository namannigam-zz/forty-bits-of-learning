...
public class BSequenceSolution {
    ...
    public static void main(String[] args) {
        ...
        IntStream.range(0, Q).forEach(q -> {
            BSequence updatedSequence = constructBSequence(s.nextInt(), bSequence);
            System.out.println(updatedSequence.getLeft().size() + updatedSequence.getRight().size());
        });
    ...
    private static List<Integer> insertInSortedSequence(List<Integer> input, Integer val, boolean asc) {
        if (asc) {
            IntStream.range(0, input.size()).filter(i -> val < input.get(i)).findFirst()
