...
/**
 * Pikachu loves battling with other Pokemon. This time he has a team of  Meowstic to fight,  of which has strength . He wants to fight with all of them  times. Team Meowstic came to know about this and now they have devised a strategy to battle against the mighty Pikachu.
 *
 ...
 * The attacking power of remaining Meowstic changes as  where  and  represents the bitwise OR of  and
 ...
 * Second line contains   space separated integers, the  of which is
 */
public class PickachuVsTeamMeawostic {
    ...
    public static void main(String[] args) {
        ...
        Arrays.stream(updatedPowers(arr, k)).mapToObj(anArrup -> anArrup + " ").forEach(System.out::print);
    }

    private static int[] updatedPowers(int arr[], int k) {
        IntStream.range(0, k).flatMap(i -> IntStream.range(1, arr.length)).forEach(j -> arr[j] = arr[j - 1] | arr[j]);
        return arr;
    }
