...
/**
 * In a company, there are  employees. Their productivities are numbered from  to , each employee is assigned with an unique number. At the end of year, their boss decided to reward  bonus money for them. The rule is that if A's productivity is greater than B's one then A's money must not less than B's money. Your task is to find the number of ways to divide  bonus money among  employees acoording to above rule. As the answer can be very large, you only need to find it modulo .
 *
 * Input Format
 *
 * The first line contains two space-separated integers  denoting the number of test cases and the modulo.
 *
 * Each test case is described in a single line containing two space-separated integers .
 *
 * Output Format
 *
