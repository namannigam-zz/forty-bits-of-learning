...
/**
 * It is well-known that the equation: x^k + y^k = z^k  has no positive solution for . But what if we consider
 * solution over a finite field. Now, the task you are given is related to that:
 *
 * Given a prime , you are asked to count the number of positive integers  doesn't exceed  s.t. modulo equation x^k +
 * y^k = z^k (modulo P)  has solution 0 < x, y, z < P .
 *
 * Input Format
 *
