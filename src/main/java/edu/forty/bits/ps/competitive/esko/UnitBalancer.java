 ...
 * If there are M units in the above line then there will be  M-1 lines in the input that defines relation between the
 * quanitites. The input format of the relationship between the units is -
 *  a = bc where a is the com.stackoverflow.nullpointer.string that denotes unit on the left hand side of the eqquation , then followed by the space
 *  is the = symbol , then followed by space is an integer value b and then followed by space is the com.stackoverflow.nullpointer.string c that
 *  denotes unit on the right hand side.
 *
 * Output
 *
 * In the output you need to print a single com.stackoverflow.nullpointer.string that denotes relation between all the units in the descending order of their value as per  the sample output.
 *
 *
 * Constraint 1 <= M <= 10
 */
...
public class UnitBalancer {
    public static void main(String args[]) throws Exception {
        ...
        Set<Equation> equations =
                IntStream.range(0, eqnBound).mapToObj(i -> Arrays.asList(scanner.nextLine().split(" ")))
        ...
        Map<String, Equation> finalUnitToEquaionMapping =
                resolvedEquations.stream().collect(Collectors.toMap(Equation::getFinalUnit, eq -> eq, (a, b) -> b));
    ...
    private static Set<Equation> resovleEquationsForMultipleSimilarKeys(Set<Equation> initialEquation) {
        ...
        eqMappings.forEach((key, value) -> {
            ...
            for(int i = 1; i < value.size(); i++) {
                ...
                finalEquations.add(new Equation(finalUnit, currentEq.getMultiplier() / initMul, currentEq.getInitialUnit()));
