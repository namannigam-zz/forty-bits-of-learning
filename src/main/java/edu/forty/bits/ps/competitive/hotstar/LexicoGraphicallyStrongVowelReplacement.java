...
public class LexicoGraphicallyStrongVowelReplacement {
    ...
    private static java.util.List<Integer> indexOfVowels(String str, int count) {
        return java.util.stream.IntStream.range(0, str.length()).filter(i -> isVowel(str.charAt(i))).boxed()
                .limit(count)
    ...
    private static int countVowels(String str) {
        return (int) java.util.stream.IntStream.range(0, str.length()).filter(i -> isVowel(str.charAt(i))).count();
    }
